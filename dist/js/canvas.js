...
GravityPoint.prototype = function (o) {
  ...
  _dragDistance: null,
  _collapsing: false,
  hitTest: function hitTest(p) {
    return this.distanceTo(p) < this.radius;
  },
  startDrag: function startDrag(dragStartPoint) {
    this._dragDistance = Vector.sub(dragStartPoint, this);
    this.dragging = true;
  },
  drag: function drag(dragToPoint) {
...
(function () {
  ...
  function mouseMove(e) {
    ...
    for (i = gravities.length - 1; i >= 0; i--) {
      g = gravities[i];
      if (!hit && g.hitTest(mouse) || g.dragging) g.isMouseOver = hit = true;else g.isMouseOver = false;
    }
  ...
  canvas.addEventListener('dblclick', doubleClick, false); // GUI
  ...
  */
  ...
  var loop = function loop() {
    ...
    bufferCtx.restore(); // パーティクルをバッファに描画
    // for (i = 0, len = particles.length; i < len; i++) {
    ...
    bufferCtx.fill();
    bufferCtx.restore(); // バッファをキャンバスに描画
...
/*  
    Gravity Points
        A PEN BY Akimitsu Hamamuro style changes by Pual Standley
*/